...
export const ruTerminalMessages: Messages = {
  ...
  'cloudSync.oauth.guide.google': 'Google: console.cloud.google.com → экран согласия (External) → Credentials → Создать OAuth client ID → тип ’Desktop app’. Скопируйте client ID; секрет не нужен.',
  ...
  'cloudSync.unlock.forgotConfirmDesc': 'Сброс синхронизации удаляет сохранённый мастер-ключ, реплики, базовые снимки и подключения провайдеров. Локальные данные не затрагиваются, но следующая синхронизация начнётся с нуля с новым мастер-ключом. Отменить нельзя.',
