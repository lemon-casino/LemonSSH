...
export const ruTerminalMessages: Messages = {
  ...
  'cloudSync.gate.warning':
    'Я понимаю, что нужно сохранить мастер-ключ и client ID OAuth-приложений. Если мастер-ключ будет забыт, можно использовать «Сброс», чтобы начать заново: локальные данные хранилища сохраняются, но сохранённые client ID, подключения провайдеров и идентификация синхронизации будут удалены, и client ID нужно будет вставить снова.',
  ...
  'cloudSync.oauth.guideTitle': 'Как заполнить',
  'cloudSync.oauth.guide.github': 'GitHub: Настройки → Настройки разработчика → OAuth Apps → New, или GitHub App. Любое имя и адрес сайта, без URI перенаправления, включите Device Flow (OAuth Apps: флажок внизу; GitHub Apps: общие настройки). Вставьте Client ID сюда.',
  'cloudSync.oauth.guide.google': '1. Откройте тот же проект в console.cloud.google.com.\n2. Библиотека → включите Google Drive API.\n3. Экран согласия оставьте в Testing; добавьте свой Gmail в Audience как test user.\n4. Data Access: добавьте drive.appdata, userinfo.email, userinfo.profile.\n5. Credentials → Создать OAuth client ID → Desktop app.\n6. Вставьте client ID сверху и client_secret (GOCSPX-) из JSON в поле секрета.\n7. Подключайтесь снова из LemonSSH; не обновляйте старую вкладку 127.0.0.1.',
  'cloudSync.oauth.guide.onedrive': 'OneDrive: Azure Portal → Регистрация приложений → New → ’Public client/native’, URI перенаправления http://localhost, включите ’Allow public client flows’. Вставьте Application (client) ID.',
  'cloudSync.unlock.forgotLink': 'Забыли мастер-ключ?',
  'cloudSync.unlock.forgotConfirmDesc': 'Сброс синхронизации удаляет сохранённый мастер-ключ, реплики, базовые снимки, подключения провайдеров и client ID OAuth-приложений. Локальные данные не затрагиваются, но следующая синхронизация начнётся с нуля с новым мастер-ключом. Отменить нельзя.',
  'cloudSync.unlock.forgotConfirmButton': 'Сбросить синхронизацию',
  'cloudSync.unlock.forgotCancel': 'Оставить',
  'cloudSync.unlock.forgotDoneToast': 'Синхронизация сброшена (записей удалено: {{count}}). Задайте новый мастер-ключ, чтобы начать заново.',
  'cloudSync.unlock.forgotFailed': 'Не удалось сбросить синхронизацию',
  'cloudSync.resetInit.button': 'Сброс',
  'cloudSync.resetInit.title': 'Сбросить синхронизацию?',
  'cloudSync.resetInit.desc': 'Забыли мастер-ключ? Сброс удаляет идентификацию синхронизации, чтобы начать заново с новым мастер-ключом.',
  'cloudSync.resetInit.itemKey': 'Удаляет сохранённый мастер-ключ и его верификатор',
  'cloudSync.resetInit.itemConnections': 'Удаляет подключения провайдеров, реплики, базовые линии и снимки',
  'cloudSync.resetInit.itemOAuth': 'Удаляет сохранённые client ID OAuth-приложений',
  'cloudSync.resetInit.itemLocalSafe': 'Локальные данные не затрагиваются; отменить нельзя',
  'cloudSync.oauth.sectionDesc': 'Зарегистрируйте OAuth-клиент типа «настольное приложение» для каждого провайдера и вставьте его client ID здесь. У Google desktop-клиента в скачанном JSON всё ещё есть client_secret; его тоже нужно вставить для обмена кода. GitHub использует Device Flow; OneDrive — PKCE с локальным redirect.',
  'cloudSync.oauth.github': 'Client ID GitHub',
  'cloudSync.oauth.google': 'Client ID Google',
  'cloudSync.oauth.googleSecret': 'Client secret Google',
  'cloudSync.oauth.googleSecretHint': 'Скопируйте client_secret (начинается с GOCSPX-) из JSON desktop-приложения. Он хранится только на этом устройстве и используется для обмена токена.',
  'cloudSync.oauth.onedrive': 'Client ID OneDrive',
  ...
  'keychain.import.dropHint': 'Перетащите сюда файлы ключей. Выберите закрытый ключ вместе с .pub, чтобы импортировать их как одну запись.',
