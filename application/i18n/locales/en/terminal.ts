...
export const enTerminalMessages: Messages = {
  ...
  'cloudSync.gate.warning':
    'I understand I should keep my master key and OAuth application client IDs somewhere safe. If I forget the master key I can use Reset init to start over: local vault data is kept, but saved client IDs, provider connections and cloud-sync identity are cleared, so I will need to paste the client IDs again.',
  ...
  'cloudSync.oauth.guide.google': '1. Open the same project in console.cloud.google.com.\n2. Library → enable Google Drive API.\n3. Keep the OAuth consent screen in Testing; add your Gmail under Audience as a test user.\n4. Data Access: add drive.appdata, userinfo.email, userinfo.profile.\n5. Credentials → Create OAuth client ID → Desktop app.\n6. Paste the client ID above and client_secret (GOCSPX-) from the downloaded JSON into the secret field.\n7. Connect again from LemonSSH; do not refresh an old 127.0.0.1 callback tab.',
  ...
  'cloudSync.unlock.forgotLink': 'Forgot your master key?',
  'cloudSync.unlock.forgotConfirmDesc': 'Resetting cloud sync removes the saved master key, replicas, baselines, provider connections and OAuth application client IDs. Your local vault data is untouched, but the next sync starts from scratch with a new master key. This cannot be undone.',
  'cloudSync.unlock.forgotConfirmButton': 'Reset cloud sync',
  'cloudSync.unlock.forgotCancel': 'Keep it',
  'cloudSync.unlock.forgotDoneToast': 'Cloud sync reset ({{count}} entries removed). Set a new master key to start again.',
  'cloudSync.unlock.forgotFailed': 'Cloud sync reset failed',
  'cloudSync.resetInit.button': 'Reset init',
  'cloudSync.resetInit.title': 'Reset cloud sync?',
  'cloudSync.resetInit.desc': 'Forgot your master key? Resetting cloud sync deletes its identity so you can start over with a new master key.',
  'cloudSync.resetInit.itemKey': 'Removes the saved master key and its verifier',
  'cloudSync.resetInit.itemConnections': 'Removes provider connections, replicas, baselines and snapshots',
  'cloudSync.resetInit.itemOAuth': 'Removes saved OAuth application client IDs',
  'cloudSync.resetInit.itemLocalSafe': 'Your local vault data is not touched; this cannot be undone',
  'cloudSync.oauth.sectionDesc': 'Register a desktop-app OAuth client for each provider, then paste its client ID here. Google desktop clients still include a client_secret in the downloaded JSON; paste that too for token exchange. GitHub uses Device Flow; OneDrive uses PKCE with a loopback redirect.',
  ...
  'cloudSync.oauth.googleSecretHint': 'Copy client_secret (starts with GOCSPX-) from the downloaded Desktop app JSON. It stays on this device and is only used to exchange tokens.',
