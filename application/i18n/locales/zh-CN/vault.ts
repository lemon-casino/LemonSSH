...
export const zhCNVaultMessages: Messages = {
  ...
  'cloudSync.oauth.guide.google': 'Google：console.cloud.google.com → OAuth 同意屏幕（外部）→ 凭据 → 创建 OAuth 客户端 ID → 类型选「桌面应用」。复制 Client ID，无需密钥。',
  ...
  'cloudSync.unlock.forgotConfirmDesc': '重置云同步会删除已保存的主密钥、副本、基线和各服务商连接。本地保险库数据不受影响，但下次同步将以新主密钥从零开始。此操作不可撤销。',
