...
func (s *ProviderFetchService) authorizeProviderRequest(request ProviderFetchRequest) (ProviderFetchRequest, error) {
	...
	if strings.HasPrefix(key, "enc:v1:") {
		if s.credentials == nil {
			return request, fmt.Errorf("Secure storage is unavailable for the AI provider key")
		}
		envelope, err := base64.StdEncoding.DecodeString(strings.TrimPrefix(key, "enc:v1:"))
		if err != nil {
			return request, fmt.Errorf("AI provider key could not be decrypted; save the key again in Settings")
		}
