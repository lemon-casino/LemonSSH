...
| SSH-02 | Shared SSH transport pool | required | `sshConnectionPool.cjs` | Go typed lease pool | Reference-counted pool implemented: immutable compatibility key (endpoint/auth fingerprint/jump chain), single-flight dial, healthy return vs discard, idle TTL and LRU eviction, shutdown; live one-auth network evidence pending | concurrency/property tests and one-auth network evidence | probe |
