...
| FND-02 | Profile store and coordination | required | renderer `localStorage`, storage events, Web Locks | Go transactional profile service | bbolt-backed transactional store implemented (revision/CAS, atomic promotion, backup manifest, receipt); single-platform evidence only, renderer persistence not wired and no cross-shell writer lease yet | key inventory, transaction/crash tests, revisions, notifications and single writer lease | probe |
