...
| FND-03 | Credential protection and profile migration | required | Electron `safeStorage`, credential/vault backup bridges | Go platform keyring plus Electron migration broker | Wails cannot directly unwrap all `enc:v1:` data | all secret fields decrypt/re-seal; no plaintext leak; atomic cutover and rollback crash matrix | not-started |
