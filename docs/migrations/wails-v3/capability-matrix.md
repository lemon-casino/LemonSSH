...
| TERM-03.1 | Telnet protocol owner | required | terminal bridge telnet paths | Go telnet service (TCP + NAWS/echo negotiation) | negotiation and auto-login parity | protocol tests, echo/NAWS and reconnect tests | not-started |
| TERM-03.2 | Serial protocol owner | required | terminal bridge serial paths | Go serial service | device enumeration and disconnect parity | device matrix, disconnect and buffer tests | not-started |
| TERM-03.3 | Mosh and ET supervised binaries | required | packaged mosh/et binaries + bridges | Go supervised binary runner | hash/arch pinning and reconnect parity | helper hash/arch checks, reconnect and cleanup tests | not-started |
| TERM-03.4 | ZMODEM/YMODEM file transfer | required | `zmodemHelper.cjs` event bridge | Go ZMODEM/YMODEM service | event boundary and file safety parity | event tests, file safety and cancellation tests | not-started |
