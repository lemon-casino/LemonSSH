...
| --- | --- | --- | --- | --- | --- | --- | --- |
| FND-01 | Wails shell and typed frontend ports | required | `electron/main.cjs`, preload, `netcattyBridge` | `cmd/netcatty`, shell-neutral runtime client | Disposable beta.12 Windows probe exists; production scaffold and three-platform evidence absent | Wails launch and contract tests on three platforms; application/domain imports remain shell-neutral | probe |
| FND-02 | Profile store and coordination | required | renderer `localStorage`, storage events, Web Locks | Go transactional profile service | Browser storage is not cross-shell portable | key inventory, transaction/crash tests, revisions, notifications and single writer lease | not-started |
| FND-03 | Credential protection and profile migration | required | Electron `safeStorage`, credential/vault backup bridges | Go platform keyring plus Electron migration broker | Disposable Windows DPAPI probe passes live interop and leak scans; macOS/Linux live keyring evidence absent | all secret fields decrypt/re-seal; no plaintext leak; atomic cutover and rollback crash matrix | probe |
...
| TERM-01 | Terminal binary data plane | required | MessagePort output/urgent input and flow control | Go data-plane service plus WebView transport | Windows WebView2 loopback WebSocket probe passes one canonical smoke; three-platform paired formal benchmark evidence is absent | zero loss/duplication, bounded RSS/queue, latency and rebind benchmarks on WebView2/WKWebView/WebKitGTK | probe |
| TERM-02 | Local PTY | required | `node-pty`, terminal worker | Go ConPTY/Unix PTY adapters | Windows signal/resize/process-tree parity | shell matrix, resize flood, Ctrl-C, Unicode, reload and child cleanup tests | not-started |
...
| SSH-01 | SSH authentication and sessions | required | `sshBridge.cjs`, ssh2 helpers and patches | Go SSH session service | MFA, certificates, agents, jump/proxy and legacy algorithm parity | compatibility lab, host-key fail-closed, cancellation and live MFA tests | not-started |
| SSH-02 | Shared SSH transport pool | required | `sshConnectionPool.cjs` | Go typed lease pool | subtle endpoint/reuse and unhealthy transport semantics | concurrency/property tests and one-auth network evidence | not-started |
| SFTP-01 | SFTP browsing | required | `sftpBridge.cjs` | Go SFTP service | sudo SFTP and non-UTF-8/raw path support | real-server matrix, cancellation, encoding, symlink and reconnect tests | not-started |
| SFTP-02 | Transfer scheduler and compressed upload | required | transfer/compress bridges and renderer scheduler | Go transfer service | pause/resume/checkpoint and high-RTT semantics | corruption/hash tests, resume matrix, renderer-close survival and throughput | not-started |
| NET-01 | Port forwarding | required | `portForwardingBridge.cjs` | Go tunnel manager on shared SSH pool | reverse/dynamic forwarding lifecycle races | local/remote/SOCKS, half-close, transport-loss and epoch tests | not-started |
...
| SYNC-01 | Vault/settings/session restore persistence | required | React hooks plus localStorage | Go profile services with frontend state adapters | hooks currently combine state, persistence and cross-window sync | differential behavior, lossless data equality and restore invariants | not-started |
