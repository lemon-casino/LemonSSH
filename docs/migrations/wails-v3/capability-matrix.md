...
| PLUG-01 | Plugin v2 contract and package store | required | internal plugin schema/packages and Electron host | Go schema/validator/package manager plus `internal/plugin/permissions` | Manifest v2 contract and permission broker implemented (schema validation, fail-closed authorization); package store, codegen drift checks and install lifecycle pending | attack corpus, codegen drift, atomic install/recovery, broker authorization and v1 rejection UX | probe |
