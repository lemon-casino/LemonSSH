...
| SSH-01 | SSH authentication and sessions | required | `sshBridge.cjs`, ssh2 helpers and patches | Go SSH session service | Dial core implemented: host-key accept-new/reject-changed policy with known-hosts file, password/key+passphrase/keyboard-interactive auth, jump chain transport with keepalive and unified close; Wails Connect now accepts jumpHosts plus socks5/http proxyUrl and surfaces keyboard-interactive challenges to the existing renderer modal; command proxies, certificates, SSH agent, and identity files still fail closed; live MFA/proxy/jump evidence pending | compatibility lab, host-key fail-closed, cancellation and live MFA tests | probe |
...
| SYNC-01 | Vault/settings/session restore persistence | required | React hooks plus localStorage | Go profile services with frontend state adapters | hostStorageAdapter still mirrors writes; Wails boot awaits hydrateReady before renderApp so empty localStorage keys are filled from Go DomainKeys first; renderer canonical cutover and domain differential evidence pending | differential behavior, lossless data equality and restore invariants | probe |
