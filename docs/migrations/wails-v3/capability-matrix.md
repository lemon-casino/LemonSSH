...
| SSH-01 | SSH authentication and sessions | required | `sshBridge.cjs`, ssh2 helpers and patches | Go SSH session service | Dial core implemented: host-key accept-new/reject-changed policy with known-hosts file, password/key+passphrase/keyboard-interactive auth, jump chain transport with keepalive and unified close; Wails Connect accepts jumpHosts, socks5/http proxyUrl, OpenSSH user certificates, and command proxies ride ProxyCommand (%h/%p) through a shell; C-grade live password/SFTP/remote-forward evidence recorded on one Debian 13 host (WV3-L106); live MFA/agent/socks5 lab and multi-host matrix still pending | compatibility lab, host-key fail-closed, cancellation and live MFA tests | probe |
...
| SFTP-01 | SFTP browsing | required | `sftpBridge.cjs` | Go SFTP service | Browsing core implemented on pkg/sftp v1.13.9 (list/stat/mkdir/rename/remove/read/write via transport-neutral RemoteFS); production ClientFS adapter wire-protocol tested against a real pkg/sftp server over net.Pipe and exposed by the Wails SFTPService over the shared SSH pool; C-grade live evidence on one Debian 13 host: readdir / plus create/write/read/cleanup roundtrip (WV3-L106); sudo SFTP, non-UTF-8/raw names and multi-host matrix pending | real-server matrix, cancellation, encoding, symlink and reconnect tests | probe |
...
| NET-01 | Port forwarding | required | `portForwardingBridge.cjs` | Go tunnel manager on shared SSH pool | Forward manager implemented (local/remote/dynamic SOCKS5, monotonic revision, subscribe+snapshot, injectable tunnel) and wired to the Wails bridge over the shared SSH pool; C-grade live remote-forward echo through a real SSH host recorded (WV3-L106); IPv6/collision lab and longer-lived tunnel soak pending | local/remote/SOCKS, half-close, transport-loss and epoch tests | probe |
...
| SYNC-01 | Vault/settings/session restore persistence | required | React hooks plus localStorage | Go profile services with frontend state adapters | Wails boot awaits hydrateReady; non-AI persistence writes go through hostStorageAdapter into Go vault/settings/sessions domains including SFTP bookmarks and transfer center; AI stores stay on localStorage until P6-05; reads remain synchronous; canonical cutover pending | differential behavior, lossless data equality and restore invariants | probe |
