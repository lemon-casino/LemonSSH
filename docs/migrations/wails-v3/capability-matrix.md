...
| SYNC-02 | Cloud and convergent sync | required | renderer CloudSyncManager and Electron cloud bridges | Go sync service | LWW merge engine and fingerprint exposed as SyncService; a Go WebDAV snapshot transport with ETag precondition conflict detection and basic auth landed (internal/platform/cloudsync); OAuth/S3 providers and key rotation still pending | encrypted fixture parity, CRDT/baseline equality, interruption and rotation rollback tests | probe |
...
| REL-01 | Packaging, signing and native resources | required | electron-builder and Electron scripts | Wails build plus platform package pipelines | Qualification packaging pipeline implemented (`scripts/package-wails.mjs`: version ldflags stamp, artifact manifest + SHA-256 checksums, purity inventory that never claims a signed installer, cross-build CGO guard) and Windows artifact launch-smoked; `scripts/sign-wails-probe.mjs` records unsigned reasons without claiming Authenticode success; signed packages and required macOS/Linux targets remain absent | Phase 6 focused package evidence plus P8-01 final signed clean-machine/package smoke on all targets | probe |
