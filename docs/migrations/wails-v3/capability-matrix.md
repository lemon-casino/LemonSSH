...
| FND-04 | Window, single-instance and app lifecycle | required | Electron `BrowserWindow`, window manager, app lifecycle | Go window/lifecycle coordinator with Wails adapter | Go window registry implemented: role single-instance, opaque tokens, close veto, dirty-editor guard, crash cleanup, session/popup role fencing; Wails PopupWindowService now opens `#/terminal-popup` and emits terminal:popup-config; multi-monitor/crash live matrix pending | multi-monitor, dirty-editor, popup/rebind, second-instance and crash tests on three platforms | probe |
...
| TERM-03.3 | Mosh and ET supervised binaries | required | packaged mosh/et binaries + bridges | Go supervised binary runner | StartMosh/StartEt resolve bundled helpers (NETCATTY_HELPER_ROOT, exe-side Resources, repo fetch layout), run the SSH bootstrap, scrape MOSH CONNECT and supervise the local client with MOSH_KEY in the environment; the handshake scrape is live-verified against a real mosh-server on Debian 13 (WV3-L106); roaming reconnect and Windows/macOS helper packaging pending | helper hash/arch checks, reconnect and cleanup tests | implemented |
| TERM-03.4 | ZMODEM/YMODEM file transfer | required | `zmodemHelper.cjs` event bridge | Go ZMODEM/YMODEM service | CRC-16 framing, ZFILE metadata safety, ZDLE escape, length-prefixed session units, sender/receiver round-trip, and serial YMODEM are implemented; CancelZmodem cancels a live serial transfer; raw lrzsz wire compatibility remains residual | event tests, file safety and cancellation tests | implemented |
...
| SFTP-01 | SFTP browsing | required | `sftpBridge.cjs` | Go SFTP service | Browsing core implemented on pkg/sftp v1.13.9 via transport-neutral RemoteFS; Wails SFTPService over the shared SSH pool; C-grade live evidence on one Debian 13 host: readdir plus write/read/cleanup roundtrip and non-UTF-8 filename tolerance (WV3-L106, WV3-L112); sudo SFTP and multi-host matrix pending | real-server matrix, cancellation, encoding, symlink and reconnect tests | probe |
| SFTP-02 | Transfer scheduler and compressed upload | required | transfer/compress bridges and renderer scheduler | Go transfer service | Chunk scheduler implemented (task identity, checkpoints, pause/resume/cancel, per-host concurrency, aggregated snapshots) and now exposed as TransferService pause/resume/cancel on the Wails bridge; startCompressedUpload zips a local folder and uploads it; high-RTT/corruption lab pending | corruption/hash tests, resume matrix, renderer-close survival and throughput | probe |
...
| SYS-02 | Tray, shortcuts and OS menus | required | global shortcut/window bridges | Go platform shell services | Shortcut registry (conflict detection, case-insensitive lookup) and tray menu state generator implemented; ShortcutService Register is on transitionBridge and fails closed because Wails v3.0.0-alpha.63 has no GlobalShortcut; native OS accelerator registration unverified | native smoke matrix including lock redaction and cleanup | probe |
| SYS-03 | Deep links, file association and context menu | required | deep-link/main/installer code | Go intent queue plus platform registration | Strict intent parser (ssh/telnet/netcatty schemes, password params rejected) and pre-ready at-most-once queue implemented; second-instance argv URLs enqueue; renderer drainDeepLinks plus onSshDeepLink consume the queue; Windows user-level registration of the three schemes via HKCU\Software\Classes is implemented behind a System-tab toggle (no elevation); macOS/Linux registration and three-platform delivery pending | cold/warm start, disabled preference, malformed URL and installed-package tests | probe |
| SYS-04 | App lock and biometric auth | required | Electron settings/runtime and native helper | Go app-lock owner plus platform auth | PBKDF2 verifier core implemented over the P2-04 provider (password sealed via keyring, verifier record password-free); Wails Enable/Unlock/Disable now persist the verifier and drive requestAppLockUnlock; Windows Hello/Touch ID still unwired | verifier migration, timeout/cancel, Touch ID/Hello and lock lifecycle tests | probe |
| SYNC-01 | Vault/settings/session restore persistence | required | React hooks plus localStorage | Go profile services with frontend state adapters | Canonical cutover landed: boot convergence promotes legacy localStorage-only values into the Go store, hydrates the local read cache from Go for fresh profiles, and heals mirror divergences toward local with a reported differential; sessions domain now included; reads stay synchronous behind hydrateReady; AI stores stay on localStorage until P6-05 (WV3-L113); live multi-machine restore evidence pending | differential behavior, lossless data equality and restore invariants | implemented |
...
| PLUG-01 | Plugin v2 contract and package store | required | internal plugin schema/packages and Electron host | Go schema/validator/package manager plus `internal/plugin/permissions` | Manifest v2 contract, permission broker, package store and declarative UI schema implemented; Install now validates manifest v2 and supports staged two-phase install with crash recovery; codegen drift, atomic on-disk recovery and v1 rejection UX pending | attack corpus, codegen drift, atomic install/recovery, broker authorization and v1 rejection UX | probe |
| PLUG-02 | WASM runtime and declarative UI | required | sandboxed BrowserWindow runtime and custom views | wazero runtime plus host-rendered UI schema and plugin permission brokers | wazero Instantiate exposed on PluginService with WASI disabled, no host imports, and entrypoint.memoryMB enforced as a per-module runtime memory cap; UI contribution model still pending; must not depend on Agent catalog | memory/time/quota/trap tests; no DOM/network/filesystem escape; shared service dispatch | probe |
