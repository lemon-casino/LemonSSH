...
| TERM-02 | Local PTY | required | `node-pty`, terminal worker | Go ConPTY/Unix PTY adapters | Go session lifecycle owner and Unix PTY backend implemented with generation fencing, resize/input/interrupt/close/reap contract; Windows ConPTY native adapter and Wails terminal wiring pending | shell matrix, resize flood, Ctrl-C, Unicode, reload and child cleanup tests | probe |
