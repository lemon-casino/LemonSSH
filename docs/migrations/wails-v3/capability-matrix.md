...
| SYS-01 | Local filesystem, dialogs and temp files | required | local FS/temp bridges | Go filesystem and platform adapters | Dedicated temp service, symlink-safe resolution, zip-slip-hardened extraction implemented and tested (Windows live); extractLocalArchive now fails closed unless FilesystemService.ExtractArchive is bound; native dialogs and UNC/long-path live matrix pending | traversal attack corpus, UNC/Unicode, temp substitution and platform tests | probe |
