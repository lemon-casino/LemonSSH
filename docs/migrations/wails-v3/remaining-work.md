...
当前台账头：`WV3-L126`。矩阵 36 行：implemented 17 / probe 12 / not-started 7 /
...
- 云同步：Go WebDAV 与 S3 sigv4 快照传输均已落地并接入渲染层 cloudSync 端口（SYNC-02，L111）；GitHub 设备流、Google/OneDrive PKCE（本地回调服务器）经原生桥接通，密钥轮换对话框走 Go CAS 事务（L126） — 已处理；活体授权证据仍缺
