...
当前台账头：`WV3-L127`。矩阵 36 行：implemented 17 / probe 12 / not-started 7 /
...
- 云同步：Go WebDAV 与 S3 sigv4 快照传输均已落地并接入渲染层 cloudSync 端口（SYNC-02，L111）；GitHub 设备流、Google/OneDrive PKCE（本地回调服务器）经原生桥接通，密钥轮换对话框走 Go CAS 事务（L126）；三个 provider 的 client ID 支持设置页运行时配置（L127），空值不再打开坏 URL — 已处理；活体授权证据仍缺
