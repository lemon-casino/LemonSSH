...
当前台账头：`WV3-L163`。矩阵 required 叶行：implemented 15 / probe 14 /
...
W04 terminal 域共享 use case（L161）、W04 SFTP 域共享 use case（L162）、W04 forward 域共享 use case（L163）。
生产 AI 下一刀是 W04 收尾（Vault 域），不是伪造 NONAI-COMPLETE。
...
- 下一刀 W04 收尾（Vault/snippet 域），然后 W05 catalog。
  W04 已落三刀（L161、L162、L163）：terminal/session/exec 抽到 `internal/app/terminaluse`，
  SFTP 抽到 `internal/app/sftpuse`（复用 terminaluse 传输 seam，无第二套池），
  forward 抽到 `internal/app/forwarduse`（同池 KindForward 租约，epoch 参数留给
  capability dispatch）。Vault facade 仍是旧 owner。
