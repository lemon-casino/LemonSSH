...
test('permission save waits, keeps dialog open on rejection, and allows retry', async () => {
    ...
    for (const name of ['window', 'document', 'navigator', 'HTMLElement', 'HTMLInputElement', 'Element', 'Node', 'NodeFilter', 'MutationObserver', 'CustomEvent', 'Event', 'localStorage', 'sessionStorage']) {
        ...
        Object.defineProperty(globalThis, name, { configurable: true, writable: true, value: name === 'window' ? dom.window : (dom.window as any)[name] });
    }
    ...
    try {
        await act(async () => root.render(React.createElement(SftpPermissionsDialog, { open: true, onOpenChange: open => { closed = !open; }, file: { name: 'a.txt', permissions: '644' } as any, onSave: () => save() })));
